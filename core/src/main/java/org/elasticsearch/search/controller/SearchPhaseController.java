...
public class SearchPhaseController extends AbstractComponent {
    ...
    public InternalSearchResponse merge(ScoreDoc[] sortedDocs, AtomicArray<? extends QuerySearchResultProvider> queryResultsArr,
        ...
        //xlogger.info("===merge===302==="+fetchResultsArr.length()+"==="+fetchResultsArr.hashCode());try { Integer.parseInt("merge"); }catch (Exception e){logger.error("===", e);}
        ...
        if (!fetchResults.isEmpty()) {
            for (ScoreDoc shardDoc : sortedDocs) {
                ...
                if (index < fetchResult.hits().internalHits().length) {
                    ...
                    //xlogger.info("===merge===377==="+searchHit.getSource());
                    hits.add(searchHit);
                }
            }
        }
